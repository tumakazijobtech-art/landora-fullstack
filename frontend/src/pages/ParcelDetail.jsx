...
export default function ParcelDetail() {
  ...
  const { canChat, startConversationForParcel } = useChat();
  ...
  // Full GIS land productivity report — fetched separately per user (free for the
  // owner/admin, otherwise gated behind a small M-Pesa fee). See
  // GET /parcels/:id/productivity-report and services/... on the backend.
  const [reportAccess, setReportAccess] = useState(null); // { unlocked, hasReport, price, productivityReport }
  const [gisPayOpen, setGisPayOpen] = useState(false);
  ...
  useEffect(() => {
    if (!parcel || !parcel.hasProductivityReport) return;
    api.getProductivityReport(parcel._id, token)
      .then(setReportAccess)
      .catch(() => {});
  }, [parcel, token]);

  async function handleMessageLandowner() {
    if (!user) {
      navigate('/login');
      return;
    }
    ...
    setChatError('');
    try {
      await startConversationForParcel(parcel._id);
    } catch (err) {
  ...
  const pr = reportAccess && reportAccess.unlocked ? reportAccess.productivityReport : null;
  ...
  return (
    <div className="section parcel-detail-section">
        ...
        <div className="parcel-layout">
          <div className="parcel-main">
            ...
            {/* Land productivity report — generated by the GIS + actuarial engine.
                Full report is gated: free for the landowner/admin, otherwise a
                small one-time M-Pesa fee unlocks it (see
                GET /parcels/:id/productivity-report on the backend). */}
            {parcel.hasProductivityReport && !pr && (
              <div className="panel">
                ...
                <div style={{ fontSize: 13, color: 'var(--s500)', marginBottom: 16 }}>
                  Generated by the GIS and actuarial engine from rainfall records, vegetation indices, soil data,
                  elevation models and road network data.
                </div>
                {parcel.productivityReport?.scoreLabel && (
                  <div className="score-hero" style={{ marginBottom: 16 }}>
                    <div className="score-hero-letter">{parcel.productivityReport.scoreLabel}</div>
                    <div style={{ fontSize: 12, color: 'var(--s500)' }}>Parcel score preview</div>
                  </div>
                )}
                <div className="info-box" style={{ marginBottom: 12 }}>
                  Unlock the full report — soil quality, rainfall reliability, market access, historical yield,
                  rainfall history, and agronomic notes — for a one-time fee
                  {reportAccess?.price ? ` of KES ${Number(reportAccess.price).toLocaleString()}` : ''}.
                </div>
                <button
                  type="button"
                  className="btn-green"
              ...
              <div className="panel" ref={applyRef}>
                {applySuccess ? (
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center' }}>
                    ...
                    <button className="btn-outline-green" onClick={handleMessageLandowner}>Message the landowner</button>
                  </div>
                ) : (
                  <>
            ...
            <div className="sidebar-card">
              ...
              {canApply && !applySuccess && (
                <div className="sidebar-cta-group">
                  <button className="btn-green" onClick={() => goToApply('lease')}>Apply to lease this parcel</button>
                  <button className="btn-outline-green" onClick={handleMessageLandowner}>Message the landowner</button>
                  {chatError && <div className="error-box" style={{ marginTop: 4 }}>{chatError}</div>}
                </div>
              )}
      ...
      <PaymentModal
        open={gisPayOpen}
        onClose={() => setGisPayOpen(false)}
        type="gis_report"
        parcelId={parcel._id}
        title="Unlock the GIS productivity report"
        description={`Full soil, rainfall, market access and historical yield data for ${parcel.title}.`}
        onSuccess={() => {
          api.getProductivityReport(parcel._id, token).then(setReportAccess).catch(() => {});
        }}
      />
    </div>
  );
